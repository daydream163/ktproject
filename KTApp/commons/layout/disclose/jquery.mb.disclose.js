...
(function($){
	...
	$.disclose = {
		...
		init: function(opt){
			...
			return this.each(function(){
				...
				el.pages.each(function(){
					...
					$(this).data("bannerId",$(this).children().first().attr("id"));
					$(this).data("done",$(this).children().first().hasClass(el.opt.doneClass));
      ...
      //find idx from banner id
      if (id) {

        for (var i = 0; i < el.pages.length; i++) {
          var pag = el.pages.eq(i);
          if (pag.data("bannerId") == id) {
            idx = pag.data("idx");
            break;
          }
        }
		...
		buildIndex:function(el){
      	...
      	var indexEl=$("<div/>").addClass("idxPage").attr("id","pageIdx_"+i).data("idx",i).data("bannerId",el.pages.eq(i).data("bannerId"));

        if($(el.pages[i]).data("done"))
          indexEl.addClass(el.opt.doneClass);

        indexEl.click(function(){
          $.disclose.goTo(el,$(this).data("bannerId"),true);
				});
